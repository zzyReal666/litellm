...
export const GuardrailsSelect: React.FC<GuardrailsSelectProps> = ({
  ...
  const resolvedPlaceholder =
    placeholder ?? t("teamPage.teamInfo.selectGuardrailsPlaceholder", { defaultValue: "Select guardrails" });
  ...
  const groups: GuardrailGroup[] = grouped
    ? [
        {
          label: t("teamPage.teamInfo.globalGuardrailsGroup", { defaultValue: "Global" }),
          icon: true,
          items: [...globalGuardrails],
        },
        {
          label: t("teamPage.teamInfo.otherGuardrailsGroup", { defaultValue: "Other" }),
          icon: false,
          items: [...otherGuardrails],
        },
      ]
              ...
              {chips.map((option) => (
                <ComboboxChip key={option.name} aria-label={option.name}>
                  {globalGuardrailNames.has(option.name) && (
                    <Globe
                      className="size-3"
                      aria-label={t("teamPage.teamInfo.globalGuardrailAriaLabel", { defaultValue: "Global guardrail" })}
                    />
                  )}
                  {option.name}
                </ComboboxChip>
