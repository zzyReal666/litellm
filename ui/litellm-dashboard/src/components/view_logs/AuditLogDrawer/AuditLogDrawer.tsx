    ...
    <div className="overflow-hidden rounded-sm border border-border bg-card">
      <div className="flex items-center justify-between border-b border-border bg-muted px-3 py-2">
        ...
        <Button
          ...
          size="icon-xs"
          onClick={handleCopy}
          title={t("viewLogs.auditLogDrawer.copyJson", { defaultValue: "Copy JSON" })}
          aria-label={t("viewLogs.auditLogDrawer.copyJson", { defaultValue: "Copy JSON" })}
        >
...
function DiffSection({ log }: { log: AuditLogEntry }) {
  ...
  if (isUpdateAction && before_value && updated_values) {
    ...
    displayBefore =
      Object.keys(changedBefore).length > 0
        ? changedBefore
        : { note: t("viewLogs.auditLogDrawer.noDifferingFields", { defaultValue: "No differing fields detected" }) };
    displayAfter =
      ...
      return (
        <div className="overflow-hidden rounded-sm border border-border bg-card">
          ...
          <p className="m-0 px-3 py-3 text-xs text-muted-foreground italic">
            {t("viewLogs.auditLogDrawer.notAvailable", { defaultValue: "N/A" })}
          </p>
        </div>
      );
    }
          ...
          <div className="overflow-hidden rounded-sm border border-border bg-card">
            ...
            <div className="space-y-1 px-3 py-3 text-xs">
              {value.token !== undefined && (
                <p>
                  <span className="text-muted-foreground">
                    {t("viewLogs.auditLogDrawer.tokenLabel", { defaultValue: "Token:" })}
                  </span>{" "}
                  {value.token ?? "N/A"}
                </p>
              )}
              {value.spend !== undefined && (
                <p>
                  <span className="text-muted-foreground">
                    {t("viewLogs.auditLogDrawer.spendLabel", { defaultValue: "Spend:" })}
                  </span>{" "}
              ...
              {value.max_budget !== undefined && (
                <p>
                  <span className="text-muted-foreground">
                    {t("viewLogs.auditLogDrawer.maxBudgetLabel", { defaultValue: "Max Budget:" })}
                  </span>{" "}
  ...
  return (
    <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
      {renderValue(t("viewLogs.auditLogDrawer.before", { defaultValue: "Before" }), displayBefore)}
      {renderValue(t("viewLogs.auditLogDrawer.after", { defaultValue: "After" }), displayAfter)}
    </div>
  );
}
    ...
    <Sheet open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <SheetContent side="right" className="w-[60%] gap-0 overflow-y-auto p-0 sm:max-w-none">
        <SheetTitle className="sr-only">
          {t("viewLogs.auditLogDrawer.title", { defaultValue: "Audit log details" })}
        </SheetTitle>
        ...
        <div className="px-6 py-5">
          <div className="mb-5 rounded-lg border border-border bg-muted p-4">
            ...
            <MetadataRow
              label={t("viewLogs.auditLogDrawer.labelTable", { defaultValue: "Table" })}
              value={tableDisplay}
            />
            <MetadataRow
              label={t("viewLogs.auditLogDrawer.labelObjectId", { defaultValue: "Object ID" })}
              value={
                <span className="inline-flex items-center gap-1 font-mono text-xs">
                  {log.object_id}
                  <CopyButton
                    value={log.object_id}
                    label={t("viewLogs.auditLogDrawer.copyObjectId", { defaultValue: "Copy object ID" })}
                  />
                </span>
              }
            />
            <MetadataRow
              label={t("viewLogs.auditLogDrawer.labelChangedBy", { defaultValue: "Changed By" })}
              value={<DefaultProxyAdminTag userId={log.changed_by} />}
            />
            <MetadataRow
              label={t("viewLogs.auditLogDrawer.labelApiKeyHash", { defaultValue: "API Key (Hash)" })}
              value={
                log.changed_by_api_key ? (
                  <span className="inline-flex items-center gap-1 font-mono text-xs break-all">
                    {log.changed_by_api_key}
                    <CopyButton
                      value={log.changed_by_api_key}
                      label={t("viewLogs.auditLogDrawer.copyApiKeyHash", { defaultValue: "Copy API key hash" })}
                    />
                  </span>
                ) : (
                  "—"
