...
export function OutputCard({ message, completionTokens, outputCost }: OutputCardProps) {
  ...
  const handleCopy = () => {
    ...
    toast.success(t("viewLogs.outputCard.outputCopied", { defaultValue: "Output copied" }));
  };
        ...
        <div className="px-4 py-3">
          {message ? (
            ...
            <span className="text-[13px] text-muted-foreground italic">
              {t("viewLogs.outputCard.noResponseData", { defaultValue: "No response data available" })}
            </span>
          )}
        </div>
      </div>
