...
export function KeyProjectField({ projectId, canDetach, pending, disabled, onToggle }: KeyProjectFieldProps) {
  ...
  return (
    <Field>
      <FieldLabel htmlFor={id}>{t("templates.keyEditView.project", { defaultValue: "Project" })}</FieldLabel>
      <Input id={id} value={display ?? ""} disabled readOnly />
      {canDetach && (
        <>
          {pending && (
            <p className="text-sm text-muted-foreground">
              {t("templates.keyEditView.detachProjectNotice", {
                defaultValue:
          ...
          <Button type="button" variant="outline" disabled={disabled} onClick={onToggle}>
            {pending
              ? t("templates.keyEditView.keepProject", { defaultValue: "Keep project" })
              : t("templates.keyEditView.detachFromProject", { defaultValue: "Detach from project" })}
          </Button>
        </>
      )}
    </Field>
