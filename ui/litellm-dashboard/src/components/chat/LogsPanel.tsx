...
function StatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs ${isFailure ? "text-destructive" : "text-success"}`}>
      ...
      {isFailure
        ? t("viewLogs.filterOptions.statusFailure", { defaultValue: "Failure" })
  ...
  if (value == null || value === "") {
    return (
      <p className="m-0 text-xs text-muted-foreground">
        {t("viewLogs.logDetailContent.responseNotAvailable", { defaultValue: "Response data not available" })}
      </p>
    );
  }
...
function LogsEmpty() {
  ...
  return (
    <div className="rounded-lg border border-dashed py-12 text-center text-sm text-muted-foreground">
      ...
      {t("chat.logsPanel.noLogs", { defaultValue: "No logs for this period" })}
    </div>
  );
}
...
function LogsError({ onRetry }: { onRetry: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed py-12 text-center text-sm text-muted-foreground">
      ...
      {t("chat.logsPanel.loadFailed", { defaultValue: "Failed to load your logs" })}
        ...
        <TableHeader>
          <TableRow className="bg-muted/50">
            <TableHead className="text-[11px] font-medium uppercase tracking-wide">
              {t("viewLogs.columns.colTime", { defaultValue: "Time" })}
            </TableHead>
            <TableHead className="text-[11px] font-medium uppercase tracking-wide">
            ...
            <TableHead className="text-right text-[11px] font-medium uppercase tracking-wide">
              {t("activityMetrics.tokens", { defaultValue: "Tokens" })}
            </TableHead>
            <TableHead className="text-right text-[11px] font-medium uppercase tracking-wide">
              {t("viewLogs.logDetailContent.labelDuration", { defaultValue: "Duration" })}
            </TableHead>
            <TableHead className="text-right text-[11px] font-medium uppercase tracking-wide">
              {t("guardrails.guardrailGardenDetail.propertyCost", { defaultValue: "Cost" })}
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
  ...
  return (
    <Dialog open={!!log} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("chat.logsPanel.requestDetails", { defaultValue: "Request details" })}</DialogTitle>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-md border bg-card p-3">
                ...
                <div className="text-sm text-foreground">
                  ...
                  {t("chat.logsPanel.tokensBreakdown", {
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="mb-0.5 text-base font-semibold tracking-tight text-foreground">
            {t("chat.logsPanel.yourLogs", { defaultValue: "Your Logs" })}
          </h2>
          <p className="m-0 text-sm text-muted-foreground">
            {t("chat.logsPanel.accountLogsOnly", { defaultValue: "Request logs for your account only" })}
          </p>
        </div>
        <div className="flex gap-1">
