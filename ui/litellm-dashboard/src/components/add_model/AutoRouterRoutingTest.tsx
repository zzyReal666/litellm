  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        {t("addModel.autoRouterRoutingTest.description", {
          defaultValue:
      ...
      <Textarea
        ...
        placeholder={t("addModel.autoRouterRoutingTest.promptPlaceholder", {
          defaultValue: "Paste a prompt an end user would send",
        })}
        rows={4}
      ...
      <div className="flex justify-end">
        ...
        >
          {state.status === "running"
            ? t("addModel.autoRouterRoutingTest.routing", { defaultValue: "Routing..." })
            : t("addModel.autoRouterRoutingTest.sendPrompt", { defaultValue: "Send Test Prompt" })}
        </Button>
      </div>
      ...
      {state.status === "failed" && (
        ...
        >
          <p className="font-medium">
            {t("addModel.autoRouterRoutingTest.routeFailed", { defaultValue: "Could not route this prompt" })}
          </p>
          <p>{state.error}</p>
        </div>
      )}
        ...
        <div data-testid="auto-router-routing-test-result">
          <div className="flex items-center gap-2 py-2 text-sm">
            <span className="text-muted-foreground">
              {t("addModel.autoRouterRoutingTest.routedTo", { defaultValue: "Routed to" })}
            </span>
            ...
            {!state.result.routed_model_configured && (
              ...
              >
                ...
                {t("addModel.autoRouterRoutingTest.unconfiguredModel", {
                  defaultValue: "This proxy has no model group by that name",
                })}
              </span>
            )}
          </div>
