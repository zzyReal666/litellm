  ...
  return (
    <div className="min-w-40 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl">
      <p className="mb-1.5 font-medium text-foreground">
        {t("caching.errorDrilldown.errorCodeFailed", {
          defaultValue: "Error code {{code}}: {{count}} failed",
          code: String(label),
          count: datum[FAILED_REQUESTS_SERIES].toLocaleString(),
        })}
      </p>
    ...
    <Card className="mt-4">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-base font-semibold">
          {t("caching.errorDrilldown.failedRequestsByErrorCode", {
            defaultValue: "Failed requests by error code: {{callType}}",
            callType,
          })}
        </CardTitle>
        <Button
          ...
          size="icon-sm"
          onClick={onClose}
          aria-label={t("caching.errorDrilldown.closeErrorBreakdown", { defaultValue: "Close error breakdown" })}
        >
          <X />
        </Button>
      </CardHeader>
      <CardContent>
        <p className="text-sm text-muted-foreground">
          {t("caching.errorDrilldown.hoverHint", {
            defaultValue: "Hover a bar to see the error classes behind that code.",
          })}
        </p>
        <BarChart
