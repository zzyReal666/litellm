...
const makeToolsetSchema = (t: TFunction) =>
  z.object({
    toolset_name: z
      .string()
      .min(1, t("mcpTools.mCPToolsetsTab.toolsetNameRequired", { defaultValue: "Please enter a toolset name" })),
    description: z.string(),
  });

type ToolsetFormValues = z.infer<ReturnType<typeof makeToolsetSchema>>;
        ...
        <span className="text-sm font-medium text-foreground flex items-center gap-2">
          ...
          {selectedSet.size > 0 && (
            <span className="ml-1 text-xs text-purple-600 font-semibold dark:text-purple-400">
              {t("mcpTools.mCPToolsetsTab.selectedCount", {
        ...
        <div className="p-2">
          {loading ? (
            ...
            <p className="text-xs text-muted-foreground px-2 py-2">
              {t("mcpTools.mCPToolsetsTab.noToolsFound", { defaultValue: "No tools found for this server." })}
            </p>
          ) : (
...
function CreateToolsetModal({ open, onClose, onSave, accessToken, initialToolset }: CreateToolsetModalProps) {
  const { t } = useTranslation();
  const form = useZodForm(makeToolsetSchema(t), {
    defaultValues: {
    ...
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[960px]">
        <DialogHeader>
          <DialogTitle>
            {initialToolset
              ? t("mcpTools.mCPToolsetsTab.editToolset", { defaultValue: "Edit Toolset" })
              : t("mcpTools.mCPToolsetsTab.newToolset", { defaultValue: "New Toolset" })}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={(event) => event.preventDefault()} className="mt-2">
          <FieldGroup className="mb-4 flex-row gap-4">
            <FormField
              ...
              name="toolset_name"
              label={t("mcpTools.mCPToolsetsTab.toolsetNameLabel", { defaultValue: "Toolset Name" })}
              className="flex-1"
            >
              {(field) => (
                <Input
                  {...field}
                  placeholder={t("mcpTools.mCPToolsetsTab.toolsetNamePlaceholder", {
                    defaultValue: "e.g. github-linear-tools",
                  })}
                />
              )}
            </FormField>
            ...
            >
              {(field) => (
                <Input
                  {...field}
                  placeholder={t("mcpTools.mCPToolsetsTab.descriptionPlaceholder", {
        ...
        <div className="flex gap-4 mt-2" style={{ minHeight: 360 }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-semibold text-foreground">
                {t("mcpTools.mCPToolsetsTab.availableTools", { defaultValue: "Available Tools" })}
              </p>
            </div>
            ...
            <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 300 }}>
              {filteredServers.length === 0 ? (
                <p className="text-muted-foreground text-sm">
                  {mcpServers.length === 0
                    ? t("mcpTools.mCPToolsetsTab.noMcpServersConfigured", { defaultValue: "No MCP servers configured" })
                    : t("mcpTools.mCPToolsetsTab.noServersMatchSearch", {
                        defaultValue: "No servers match your search",
                      })}
                </p>
              ) : (
          ...
          <div className="w-72 shrink-0">
            <p className="text-sm font-semibold text-foreground mb-2 block">
              {t("mcpTools.mCPToolsetsTab.yourToolset", { defaultValue: "Your Toolset" })}{" "}
              <span className="text-xs font-normal text-muted-foreground">
                {t("mcpTools.mCPToolsetsTab.toolCount", {
            ...
            <div className="space-y-1 overflow-y-auto" style={{ maxHeight: 340 }}>
              {selectedTools.length === 0 ? (
                <p className="text-muted-foreground text-sm">
                  {t("mcpTools.mCPToolsetsTab.noToolsAdded", { defaultValue: "No tools added yet" })}
                </p>
              ) : (
        ...
        <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-border">
          ...
          <Button onClick={() => void form.handleSubmit(handleSubmit)()} disabled={saving} aria-busy={saving}>
            ...
            {initialToolset
              ? t("mcpTools.mCPToolsetsTab.saveChanges", { defaultValue: "Save Changes" })
              : t("mcpTools.mCPToolsetsTab.createToolset", { defaultValue: "Create Toolset" })}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">
        <Trans i18nKey="mcpTools.mCPToolsetsTab.noToolsets" defaults="No toolsets yet" />
      </div>
      <div className="text-sm text-muted-foreground">
        {t("mcpTools.mCPToolsetsTab.noToolsetsHint", {
          defaultValue: "Create a toolset to give keys and teams a curated set of MCP tools.",
        })}
      </div>
    </div>
  );
  ...
  return (
    <div className="mb-6 rounded-lg border border-border bg-muted px-5 py-4">
      <p className="text-sm font-medium text-foreground mb-1">
        {t("mcpTools.mCPToolsetsTab.howToolsetsWorkTitle", { defaultValue: "How toolsets work" })}
      </p>
      <p className="text-sm text-muted-foreground mb-3">
        {t("mcpTools.mCPToolsetsTab.howToolsetsWorkDesc1", {
          defaultValue: "Create a toolset, assign it to a key via",
        })}{" "}
        <span className="font-medium text-foreground">
          {t("mcpTools.mCPToolsetsTab.howToolsetsWorkPath", { defaultValue: "API Keys → Edit Key → MCP Servers" })}
        </span>
        {t("mcpTools.mCPToolsetsTab.howToolsetsWorkDesc2", {
          defaultValue: ", then point your MCP client at the toolset URL. The client only sees the tools you picked.",
        })}
      </p>
      <div className="text-xs text-muted-foreground mb-1">
        {t("mcpTools.mCPToolsetsTab.configLabel", { defaultValue: "Claude Code / Cursor config" })}
      </div>
...
export function MCPToolsetsTab({ accessToken, userRole }: MCPToolsetsTabProps) {
  ...
  const handleCreate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    toast.success(t("mcpTools.mCPToolsetsTab.toolsetCreated", { defaultValue: "Toolset created" }));
  ...
  const handleUpdate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    toast.success(t("mcpTools.mCPToolsetsTab.toolsetUpdated", { defaultValue: "Toolset updated" }));
    queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
    setEditToolset(null);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t("mcpTools.mCPToolsetsTab.toolsetDeleted", { defaultValue: "Toolset deleted" }));
      queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
      setDeleteId(null);
    } finally {
      setDeleting(false);
  ...
  const columns = React.useMemo(() => {
    ...
    return getMCPToolsetTableColumns(columnDeps);
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-medium text-foreground">
            {t("mcpTools.mCPToolsetsTab.pageTitle", { defaultValue: "MCP Toolsets" })}
          </h3>
          <p className="text-muted-foreground text-sm">
            {t("mcpTools.mCPToolsetsTab.pageSubtitle", {
              defaultValue:
      ...
      <DataTable
        ...
        isLoading={isLoading}
        loadingMessage={t("mcpTools.mCPToolsetsTab.loadingToolsets", { defaultValue: "Loading toolsets..." })}
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {t("mcpTools.mCPToolsetsTab.deleteToolsetTitle", { defaultValue: "Delete Toolset" })}
            </DialogTitle>
          </DialogHeader>
          <p>
            {t("mcpTools.mCPToolsetsTab.deleteToolsetConfirm", {
              defaultValue:
