...
const SCOPE_OPTIONS = [
  { value: "global", labelKey: "mcpTools.envVarsSection.scopeInstance", label: "Instance" },
  { value: "user", labelKey: "mcpTools.envVarsSection.scopePerUser", label: "Per-user" },
];
    ...
    <div className="rounded-lg border border-border bg-muted p-4">
      <div className="flex items-center gap-2 mb-1">
        <strong className="text-sm font-semibold">
          {t("mcpTools.envVarsSection.sectionTitle", { defaultValue: "Variables" })}
        </strong>
        <SimpleTooltip
          content={
            <>
              {t("mcpTools.envVarsSection.tooltipDefine", {
                defaultValue: "Define variables you can interpolate in Static Headers or Authentication using",
              })}{" "}
              <code>{"${VAR_NAME}"}</code>. <br />
              <b>{t("mcpTools.envVarsSection.scopeInstance", { defaultValue: "Instance" })}</b>:{" "}
              {t("mcpTools.envVarsSection.tooltipInstance", {
                defaultValue: "admin-defined value used for every user.",
              })}
              <br />
              <b>{t("mcpTools.envVarsSection.scopePerUser", { defaultValue: "Per-user" })}</b>:{" "}
              {t("mcpTools.envVarsSection.tooltipPerUser", {
                defaultValue:
      ...
      <span className="mb-3 block text-xs text-muted-foreground">
        {t("mcpTools.envVarsSection.referenceHint", {
          defaultValue: "Reference these in Static Headers or Authentication as",
        })}{" "}
        <code>{"${VAR_NAME}"}</code>. {t("mcpTools.envVarsSection.exampleLabel", { defaultValue: "For example:" })}{" "}
      ...
      <div className="space-y-2">
        {fields.length > 0 && (
          <div className="flex gap-3 px-1 text-xs font-medium text-muted-foreground uppercase tracking-wide">
            <div style={{ flex: 1 }}>{t("mcpTools.envVarsSection.colVarName", { defaultValue: "Variable Name" })}</div>
            <div style={{ flex: 1 }}>
              {t("mcpTools.envVarsSection.colValueDesc", { defaultValue: "Value / Description" })}
            </div>
            <div style={{ width: 160 }}>{t("mcpTools.envVarsSection.colScope", { defaultValue: "Scope" })}</div>
            <div style={{ width: 24 }} />
          </div>
        )}
          ...
          <div key={item.id} className="flex gap-3 items-start">
              ...
              rules={{
                validate: {
                  required: requiredRule(
                    t("mcpTools.envVarsSection.varNameRequired", { defaultValue: "Variable name is required" }),
                  ),
                  pattern: matchesPattern(
                    VARIABLE_NAME_PATTERN,
                    t("mcpTools.envVarsSection.varNamePattern", {
            ...
            >
              {(control) => (
                <Input
                  {...textControl(control)}
                  placeholder={t("mcpTools.envVarsSection.varNamePlaceholder", { defaultValue: "e.g. DB_PROTOCOL" })}
            ...
            <MountedFormField name={["env_vars", String(index), "scope"]} className="mb-0 w-40" defaultValue="global">
              {(control) => (
                <Select {...selectControl<string>(control)} items={localizeSelectItems(SCOPE_OPTIONS, t)}>
        ...
        <Button variant="outline" className="w-full border-dashed" onClick={() => append({ scope: "global" })}>
          <Plus />
          {t("mcpTools.envVarsSection.addVariable", { defaultValue: "Add Variable" })}
        </Button>
      </div>
    </div>
  );
      ...
      <MountedFormField name={["env_vars", String(index), "description"]} className="mb-0">
        {(control) => (
          <InputGroup>
            <InputGroupAddon>
              <SimpleTooltip
                content={t("mcpTools.envVarsSection.perUserHintTooltip", {
                  defaultValue:
              ...
              >
                <span className="text-xs text-muted-foreground cursor-help whitespace-nowrap">
                  ...
                  {t("mcpTools.envVarsSection.hintLabel", { defaultValue: "Hint" })}
                </span>
              </SimpleTooltip>
            </InputGroupAddon>
            <InputGroupInput
              {...textControl(control)}
              placeholder={t("mcpTools.envVarsSection.perUserPlaceholder", { defaultValue: "e.g. Your DB username" })}
  ...
  return (
    <MountedFormField name={["env_vars", String(index), "value"]} className="mb-0">
      {(control) => (
        <Input
          {...textControl(control)}
          placeholder={t("mcpTools.envVarsSection.valuePlaceholder", { defaultValue: "e.g. postgresql" })}
          className="rounded-md font-mono"
        />
      )}
    </MountedFormField>
