  ...
  const handleCreate = async (values: Record<string, unknown>) => {
    ...
    try {
      if (accessToken != null) {
        ...
        if (isAdmin) {
          toast.success(
            t("mcpTools.createMcpServer.createSuccess", { defaultValue: "MCP Server created successfully" }),
          );
        } else {
          toast.success(
            t("mcpTools.createMcpServer.submitSuccess", { defaultValue: "MCP Server submitted for admin review" }),
            {
              description: t("mcpTools.createMcpServer.submitSuccessDesc", {
                defaultValue: "Once an admin approves it, the server will appear in your MCP Servers list.",
              }),
            },
          );
        }
      ...
      toast.fromError(
        isAdmin
          ? t("mcpTools.createMcpServer.createFailed", {
              reason,
              defaultValue: "Error creating MCP Server: {{reason}}",
            })
          : t("mcpTools.createMcpServer.submitFailed", {
              reason,
              defaultValue: "Error submitting MCP Server: {{reason}}",
            }),
      );
    } finally {
      setIsLoading(false);
      ...
      <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
        <DialogHeader>
          <div className="flex items-center gap-3 border-b border-border pb-4">
            ...
            <img
              src={mcpLogoImg}
              alt={t("mcpTools.createMcpServer.mcpLogoAlt", { defaultValue: "MCP Logo" })}
              className="size-5 object-contain"
            />
            <DialogTitle className="text-xl font-semibold">
              {isAdmin
                ? t("mcpTools.createMcpServer.modalTitleAdmin", { defaultValue: "Add New MCP Server" })
                : t("mcpTools.createMcpServer.modalTitleUser", { defaultValue: "Submit MCP Server for Review" })}
            </DialogTitle>
          </div>
        </DialogHeader>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {!isAdmin && (
                  <div className="rounded-md bg-info/10 border border-info/20 px-4 py-3 text-sm text-info">
                    {t("mcpTools.createMcpServer.nonAdminBannerText", {
                      defaultValue:
                ...
                <div className="grid grid-cols-1 gap-6">
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground flex items-center">
                        {t("mcpTools.createMcpServer.serverNameLabel", { defaultValue: "MCP Server Name" })}
                        <SimpleTooltip
                          content={t("mcpTools.createMcpServer.serverNameTooltip", {
                            defaultValue:
                    ...
                    {(control) => (
                      <Input
                        ...
                        placeholder={t("mcpTools.createMcpServer.serverNamePlaceholder", {
                          defaultValue: "e.g., GitHub_MCP, Zapier_MCP, etc.",
                        })}
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground flex items-center">
                        {t("mcpTools.createMcpServer.aliasLabel", { defaultValue: "Alias" })}
                        <SimpleTooltip
                          content={t("mcpTools.createMcpServer.aliasTooltip", {
                            defaultValue:
                    ...
                    {(control) => (
                      <Input
                        ...
                        placeholder={t("mcpTools.createMcpServer.descriptionPlaceholder", {
                          defaultValue: "Brief description of what this server does",
                        })}
                        className="rounded-lg border-border focus:border-info focus:ring-ring"
                      />
                    )}
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground">
                        {t("mcpTools.createMcpServer.sourceUrlLabel", { defaultValue: "GitHub / Source URL" })}
                      </span>
                    }
                    name="source_url"
                  >
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground">
                        {t("mcpTools.createMcpServer.transportLabel", { defaultValue: "Transport Type" })}
                      </span>
                    }
                    name="transport"
                    required
                    rules={{
                      validate: {
                        required: requiredRule(
                          t("mcpTools.createMcpServer.transportRequired", {
                            defaultValue: "Please select a transport type",
                          }),
                        ),
                      },
                    }}
                  ...
                  {(transportType === "http" || transportType === "sse") && (
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground">
                          {t("mcpTools.createMcpServer.serverUrlLabel", { defaultValue: "MCP Server URL" })}
                        </span>
                      }
                      name="url"
                      required
                      rules={{
                        validate: {
                          required: requiredRule(
                            t("mcpTools.createMcpServer.serverUrlRequired", {
                              defaultValue: "Please enter a server URL",
                            }),
                          ),
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground flex items-center">
                        {t("mcpTools.mcpServerEdit.maxConcurrentRequestsLabel", {
                          defaultValue: "Max Concurrent Requests (optional)",
                        })}
                        <SimpleTooltip
                          content={t("mcpTools.mcpServerEdit.maxConcurrentRequestsTooltip", {
                            defaultValue:
                    ...
                    <Collapsible defaultOpen className="mb-4">
                      <CollapsibleTrigger className="group flex w-full items-center justify-between gap-4 py-2 text-left">
                        <span className="text-sm font-semibold text-foreground">
                          {t("mcpTools.createMcpServer.authSettingsLabel", { defaultValue: "Authentication settings" })}
                        </span>
                      ...
                      <CollapsibleContent keepMounted className="space-y-6 pt-2">
                        <MountedFormField
                          label={t("mcpTools.createMcpServer.authLabel", { defaultValue: "Authentication" })}
                          name="auth_type"
                          required
                          rules={{
                            validate: {
                              required: requiredRule(
                                t("mcpTools.createMcpServer.authTypeRequired", {
                                  defaultValue: "Please select an auth type",
                                }),
                              ),
                            },
                          }}
                        ...
                        {shouldShowAuthValueField && (
                          <MountedFormField
                            label={
                              <span className="text-sm font-medium text-foreground flex items-center">
                                {t("mcpTools.createMcpServer.authValueLabel", { defaultValue: "Authentication Value" })}
                                <SimpleTooltip
                                  content={t("mcpTools.createMcpServer.authValueTooltip", {
                                    defaultValue:
                            ...
                            rules={{
                              validate: {
                                notWhitespace: notOnlyWhitespace(
                                  t("mcpTools.createMcpServer.authValueEmpty", {
                                    defaultValue: "Authentication value cannot be empty whitespace",
                                  }),
                                ),
                              },
                            }}
                            ...
                            {(control) => (
                              <PasswordInput
                                ...
                                placeholder={t("mcpTools.createMcpServer.authValuePlaceholder", {
                                  defaultValue: "Enter token or secret",
                                })}
                ...
                <div className="flex items-center justify-end space-x-3 pt-6 border-t border-border">
                  ...
                  <Button type="submit" disabled={isLoading} aria-busy={isLoading}>
                    ...
                    {isLoading
                      ? t("mcpTools.createMcpServer.creating", { defaultValue: "Creating..." })
                      : t("mcpTools.createMcpServer.addButton", { defaultValue: "Add MCP Server" })}
                  </Button>
                </div>
              </form>
            </MountedFormProvider>
