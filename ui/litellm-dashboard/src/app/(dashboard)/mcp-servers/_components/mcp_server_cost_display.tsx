        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-border bg-muted p-4">
            <p className="text-sm text-muted-foreground">
              {t("mcpTools.mcpServerCostDisplay.noCostConfig", {
                defaultValue:
    ...
    <div className="mt-6 border-t border-border pt-6">
      <div className="space-y-4">
          ...
          costConfig?.default_cost_per_query !== null && (
            <div>
              <p className="text-sm font-medium">
                {t("mcpTools.mcpServerCostDisplay.defaultCostPerQuery", { defaultValue: "Default Cost per Query" })}
              </p>
        ...
        {hasToolCosts && costConfig?.tool_name_to_cost_per_query && (
          <div>
            <p className="text-sm font-medium">
              {t("mcpTools.mcpServerCostDisplay.toolSpecificCosts", { defaultValue: "Tool-Specific Costs" })}
            </p>
                  ...
                  cost !== undefined && (
                    <div key={toolName} className="flex items-center justify-between rounded-lg bg-muted p-3">
                      ...
                      <p className="font-mono text-sm">
                        {t("mcpTools.mcpServerCostDisplay.costPerQuery", {
                          cost: cost.toFixed(4),
                          defaultValue: "${{cost}} per query",
                        })}
                      </p>
                    </div>
                  ),
        ...
        <div className="mt-4 rounded-lg border border-border bg-muted p-4">
          <p className="text-sm font-medium">
            {t("mcpTools.mcpServerCostDisplay.costSummary", { defaultValue: "Cost Summary:" })}
          </p>
            ...
            {hasToolCosts && costConfig?.tool_name_to_cost_per_query && (
              <p className="text-sm text-muted-foreground">
                {t("mcpTools.mcpServerCostDisplay.toolsWithCustomPricing", {
                  ...
                  defaultValue: "• {{count}} tool(s) with custom pricing",
                })}
              </p>
            )}
          </div>
