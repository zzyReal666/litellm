...
const MCPToolsViewer = ({
  ...
  const {
    ...
    queryFn: async () => {
      if (!accessToken)
        throw new Error(t("mcpTools.mcpTools.accessTokenRequired", { defaultValue: "Access Token required" }));
        ...
        <div className="grid h-auto w-full grid-cols-4 gap-4">
          ...
          <div className="col-span-1 flex flex-col bg-muted p-4">
            <h2 className="mt-2 mb-6 text-xl font-semibold">
              {t("mcpTools.mcpTools.title", { defaultValue: "MCP Tools" })}
            </h2>
                ...
                <div className="mb-4 rounded-lg border border-border bg-card p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <p className="text-sm font-medium">
                        {t("mcpTools.mcpTools.additionalHeaders", { defaultValue: "Additional Headers" })}
                      </p>
                    </div>
                    <Button variant="link" size="sm" onClick={() => setShowHeaderInput(!showHeaderInput)}>
                      {showHeaderInput
                        ? t("mcpTools.mcpTools.hide", { defaultValue: "Hide" })
                        : t("mcpTools.mcpTools.configure", { defaultValue: "Configure" })}
                    </Button>
                  </div>

                  {!showHeaderInput && Object.keys(passthroughHeaders).length === 0 && (
                    <p className="text-xs text-muted-foreground">
                      {t("mcpTools.mcpTools.headersRequiredHint", {
                        defaultValue: 'This server requires additional headers. Click "Configure" to provide values.',
                      })}
                    </p>
                  )}
                  ...
                  {showHeaderInput && (
                    <div className="mt-3 space-y-2">
                      {extraHeaders?.map((headerName) => (
                        <div key={headerName}>
                          ...
                          <InputGroup className="w-full">
                            ...
                            <InputGroupInput
                              placeholder={t("mcpTools.mcpTools.enterHeaderPlaceholder", {
                                header: headerName,
                                defaultValue: "Enter {{header}}",
                              })}
                      ...
                      >
                        {t("mcpTools.mcpTools.loadTools", { defaultValue: "Load Tools" })}
                      </Button>
                    </div>
                  )}
                  ...
                  {!showHeaderInput && Object.keys(passthroughHeaders).length > 0 && (
                    <div className="mt-2">
                      <p className="flex items-center text-xs text-muted-foreground">
                        ...
                        {t("mcpTools.mcpTools.headersConfigured", {
                          count: Object.keys(passthroughHeaders).length,
                          defaultValue: "{{count}} header(s) configured",
                        })}
                      </p>
                    </div>
                  )}
                ...
                {usesBrowserHeldToken && !oauthToken && (
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    ...
                    <p className="mb-1 text-xs font-medium">
                      {t("mcpTools.mcpTools.authRequired", { defaultValue: "Authentication required" })}
                    </p>
                    <p className="mb-3 text-xs text-muted-foreground">
                      {t("mcpTools.mcpTools.authenticateToViewTools", {
                        defaultValue: "Authenticate to view available tools",
                      })}
                    </p>
                    <Button
                      size="sm"
                    ...
                    >
                      {t("mcpTools.mcpTools.authorize", { defaultValue: "Authorize" })}
                    </Button>
                  ...
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    ...
                    <p className="mb-3 text-xs text-muted-foreground">
                      {t("mcpTools.mcpTools.authenticateWithUpstream", {
                        defaultValue: "Authenticate with the upstream provider to view available tools",
                      })}
                    </p>
                    <Button
                      size="sm"
                    ...
                    {toolsAreaLoading && (
                      <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card py-8">
                        ...
                        <p className="text-xs font-medium">
                          {t("mcpTools.mcpTools.loadingTools", { defaultValue: "Loading tools..." })}
                        </p>
                      </div>
                    )}
                    ...
                    {(mcpToolsResponse?.error || mcpToolsError) && !toolsAreaLoading && !toolsData.length && (
                      <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-xs text-destructive">
                        <p className="font-medium">
                          {t("mcpTools.mcpTools.errorPrefix", { defaultValue: "Error: " })}
                    ...
                    {!toolsAreaLoading &&
                      ...
                      (!toolsData || toolsData.length === 0) && (
                        <div className="rounded-lg border border-border bg-card p-4 text-center">
                          ...
                          <p className="mb-1 text-xs font-medium">
                            {t("mcpTools.mcpTools.noToolsAvailable", { defaultValue: "No tools available" })}
                          </p>
                          <p className="text-xs text-muted-foreground">
                      ...
                      <>
                        {filteredTools.length === 0 ? (
                          <div className="rounded-lg border border-border bg-card p-4 text-center">
                            ...
                            <p className="mb-1 text-xs font-medium">
                              {t("mcpTools.mcpTools.noToolsFound", { defaultValue: "No tools found" })}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {t("mcpTools.mcpTools.noToolsMatchSearch", {
                                term: toolSearchTerm,
                                defaultValue: 'No tools match "{{term}}"',
                              })}
                            </p>
                          </div>
                        ) : (
                                ...
                                <div className="flex items-start space-x-2">
                                  {tool.mcp_info.logo_url && (
                                    <img
                                      ...
                                      alt={t("mcpTools.mcpTools.serverLogoAlt", {
                                ...
                                {selectedTool?.name === tool.name && (
                                  <div className="mt-2 border-t border-border pt-2">
                                    <div className="flex items-center text-xs font-medium text-primary">
                                      ...
                                      {t("mcpTools.mcpTools.selected", { defaultValue: "Selected" })}
                                    </div>
                                  </div>
                                )}
                              </div>
          ...
          <div className="col-span-3 flex flex-col">
            <div className="flex items-center justify-between border-b border-border p-4">
              <h2 className="mb-0 text-xl font-semibold">
                {t("mcpTools.mcpTools.testingPlaygroundTitle", { defaultValue: "Tool Testing Playground" })}
              </h2>
            </div>
              ...
              {!selectedTool ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                  ...
                  <p className="mb-2 text-lg font-medium">
                    {t("mcpTools.mcpTools.selectToolToTest", { defaultValue: "Select a Tool to Test" })}
                  </p>
                  <p className="max-w-md text-center text-sm">
                    {t("mcpTools.mcpTools.selectToolHint", {
                      defaultValue:
