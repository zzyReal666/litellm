...
const IngestSuccessAlert: React.FC<{ ingestResults: RAGIngestResponse[] }> = ({ ingestResults }) => {
  ...
  return (
    <Alert variant="success">
      <CircleCheck />
      <AlertTitle>
        {t("vectorStoreManagement.createVectorStore.successMessage", {
      ...
      <AlertDescription>
        <div>
          <p>
            <strong>
              {t("vectorStoreManagement.createVectorStore.vectorStoreIdLabel", { defaultValue: "Vector Store ID:" })}
            </strong>{" "}
          ...
          <p>
            <strong>
              {t("vectorStoreManagement.createVectorStore.documentsIngestedLabel", {
                defaultValue: "Documents Ingested:",
              })}
            </strong>{" "}
...
const CreateVectorStore: React.FC<CreateVectorStoreProps> = ({ accessToken, onSuccess }) => {
  ...
  const isSupportedDocument = (file: File): boolean => {
    if (!ACCEPTED_DOCUMENT_TYPES.includes(file.type)) {
      toast.error(
        t("vectorStoreManagement.createVectorStore.unsupportedFileType", {
          name: file.name,
          defaultValue: "{{name}} is not a supported file type. Please upload PDF, TXT, DOCX, or MD files.",
        }),
      );
      return false;
    }
    if (file.size >= MAX_DOCUMENT_BYTES) {
      toast.error(
        t("vectorStoreManagement.createVectorStore.fileTooLarge", {
          name: file.name,
          defaultValue: "{{name}} must be smaller than 50MB!",
        }),
      );
      return false;
    }
  ...
  const handleCreateVectorStore = async () => {
    if (documents.length === 0) {
      toast.warning(
        t("vectorStoreManagement.createVectorStore.uploadAtLeastOne", {
    ...
    if (!selectedProvider) {
      toast.warning(
        t("vectorStoreManagement.createVectorStore.selectProvider", { defaultValue: "Please select a provider" }),
      );
      return;
    }
    ...
    if (selectedProvider === "s3_vectors") {
      ...
      if (bucketName && bucketName.length < 3) {
        toast.warning(
          t("vectorStoreManagement.createVectorStore.bucketNameMinLength", {
    ...
    try {
      ...
      toast.success(
        t("vectorStoreManagement.createVectorStore.createSuccess", {
          count: results.length,
          vectorStoreId,
          defaultValue:
            "Successfully created vector store with {{count}} document(s). Vector Store ID: {{vectorStoreId}}",
        }),
      );
      ...
      toast.fromError(
        t("vectorStoreManagement.createVectorStore.createFailed", {
          error: String(error),
      ...
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-medium">
            {t("vectorStoreManagement.createVectorStore.pageTitle", { defaultValue: "Create Vector Store" })}
          </h3>
          ...
          <CardContent>
            <div className="mb-4">
              <p className="font-medium">
                {t("vectorStoreManagement.createVectorStore.step1Title", { defaultValue: "Step 1: Upload Documents" })}
              </p>
              <p className="text-sm text-muted-foreground block mt-1">
                {t("vectorStoreManagement.createVectorStore.step1Subtitle", {
                  defaultValue: "Upload one or more documents (PDF, TXT, DOCX, MD). Maximum file size: 50MB per file.",
                })}
              </p>
            </div>
            <label
        ...
        {documents.length > 0 && (
          <Card>
            <CardContent>
              <div className="mb-4">
                <p className="font-medium">
                  {t("vectorStoreManagement.createVectorStore.uploadedDocuments", {
                    count: documents.length,
                    defaultValue: "Uploaded Documents ({{count}})",
                  })}
                </p>
              </div>
          ...
          <CardContent className="space-y-4">
            ...
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="vector-store-name">
                  {labelWithHint(
                    ...
                    t("vectorStoreManagement.createVectorStore.vectorStoreNameTooltip", {
              ...
              <Field>
                <FieldLabel htmlFor="vector-store-description">
                  {labelWithHint(
                    ...
                    t("vectorStoreManagement.createVectorStore.descriptionTooltip", {
              ...
              <Field>
                <FieldLabel htmlFor="vector-store-provider">
                  {labelWithHint(
                    t("vectorStoreManagement.createVectorStore.providerLabel", { defaultValue: "Provider" }),
                    t("vectorStoreManagement.createVectorStore.providerTooltip", {
            ...
            <div className="flex justify-end">
              ...
              >
                ...
                {isCreating
                  ? t("vectorStoreManagement.createVectorStore.creating", { defaultValue: "Creating Vector Store..." })
                  : t("vectorStoreManagement.createVectorStore.pageTitle", { defaultValue: "Create Vector Store" })}
              </Button>
            </div>
          </CardContent>
