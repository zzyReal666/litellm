...
export const VectorStoreTester: React.FC<VectorStoreTesterProps> = ({ vectorStoreId, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      toast.warning(
        t("vectorStoreManagement.vectorStoreTester.enterSearchQuery", { defaultValue: "Please enter a search query" }),
      );
      return;
    }
  ...
  const clearHistory = () => {
    ...
    setExpandedResults({});
    toast.success(
      t("vectorStoreManagement.vectorStoreTester.historyCleared", { defaultValue: "Search history cleared" }),
    );
  };
        ...
        <div className="flex items-center justify-between border-b p-4">
          <div className="flex items-center">
            ...
            <h4 className="text-base font-medium text-foreground">
              {t("vectorStoreManagement.vectorStoreTester.title", { defaultValue: "Test Vector Store" })}
            </h4>
          </div>
          {searchHistory.length > 0 && (
            <Button variant="outline" size="sm" onClick={clearHistory}>
              {t("vectorStoreManagement.vectorStoreTester.clearHistory", { defaultValue: "Clear History" })}
            </Button>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto p-4 pb-0">
          {searchHistory.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="text-sm">
                {t("vectorStoreManagement.vectorStoreTester.emptyPrompt", {
                  defaultValue: "Test your vector store by entering a search query below",
                })}
              </p>
            </div>
          ) : (
                    ...
                    <div className="inline-block max-w-[80%] rounded-lg bg-muted p-3 shadow-xs ring-1 ring-foreground/10">
                      <div className="mb-1 flex items-center gap-2">
                        <strong className="text-sm">
                          {t("vectorStoreManagement.vectorStoreTester.queryLabel", { defaultValue: "Query" })}
                        </strong>
                  ...
                  <div className="text-left">
                    <div className="inline-block max-w-[80%] rounded-lg bg-card p-3 shadow-xs ring-1 ring-foreground/10">
                      <div className="mb-2 flex items-center gap-2">
                        ...
                        <strong className="text-sm">
                          {t("vectorStoreManagement.vectorStoreTester.resultsLabel", {
                            defaultValue: "Vector Store Results",
                          })}
                        </strong>
                        {entry.response && (
                          <span className="rounded-sm bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                            {t("vectorStores.vectorStoreTester.resultsCount", {
                      ...
                      {entry.response && entry.response.data && entry.response.data.length > 0 ? (
                                ...
                                >
                                  <div className="flex items-center">
                                    ...
                                    <span className="text-sm font-medium">
                                      {t("vectorStores.vectorStoreTester.resultLabel", {
                                        index: resultIndex + 1,
                        ...
                        >
                          {entry.error
                            ? t("vectorStores.vectorStoreTester.searchFailed", {
                                error: entry.error,
                                defaultValue: "Search failed: {{error}}",
                              })
            ...
            <div className="flex-1">
              <Textarea
                ...
                placeholder={t("vectorStoreManagement.vectorStoreTester.searchPlaceholder", {
                  defaultValue: "Enter your search query... (Shift+Enter for new line)",
                })}
                disabled={isLoading}
                rows={1}
