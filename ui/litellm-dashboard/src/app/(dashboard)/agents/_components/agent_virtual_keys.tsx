  ...
  return (
    <div className="mt-6">
      <h4 className="text-base font-semibold text-foreground">
        {t("nav.virtualKeys", { defaultValue: "Virtual Keys" })}
      </h4>
      {isLoading ? (
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {t("agentsPage.agentVirtualKeys.noKeyAssigned", {
            defaultValue: "No virtual key assigned to this agent.",
          })}
        </p>
      ) : (
          ...
          {keys.map((key) => (
            <div key={key.token} className="flex items-center gap-3 rounded-sm border border-border px-3 py-2">
              ...
              <span className="text-sm font-medium text-foreground">
                {key.key_alias || t("agentsPage.agentVirtualKeys.unnamedKey", { defaultValue: "Unnamed key" })}
              </span>
