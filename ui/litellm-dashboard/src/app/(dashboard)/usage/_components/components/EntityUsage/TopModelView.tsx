...
export default function TopModelView({ topModels, topModelsLimit, setTopModelsLimit }: TopModelViewProps) {
  ...
  const columns = [
    {
      header: t("usagePage.topModelView.colModel", { defaultValue: "Model" }),
      accessorKey: "key",
      cell: (info: any) => info.getValue() || "-",
    },
    {
      header: t("usagePage.topModelView.colSpendUsd", { defaultValue: "Spend (USD)" }),
      accessorKey: "spend",
      meta: { numeric: true },
      cell: (info: any) => <MoneyCell value={info.getValue()} decimals={2} />,
    },
    {
      header: t("usagePage.topModelView.colSuccessful", { defaultValue: "Successful" }),
      accessorKey: "successful_requests",
      meta: { numeric: true },
      cell: (info: any) => <span className="text-success">{info.getValue()?.toLocaleString() || 0}</span>,
    },
    {
      header: t("usagePage.topModelView.colFailed", { defaultValue: "Failed" }),
      accessorKey: "failed_requests",
      meta: { numeric: true },
      cell: (info: any) => <span className="text-destructive">{info.getValue()?.toLocaleString() || 0}</span>,
    },
    {
      header: t("usagePage.topModelView.colTokens", { defaultValue: "Tokens" }),
      accessorKey: "tokens",
      meta: { numeric: true },
    ...
    <>
      <div className="mb-4 flex justify-between items-center">
        <Tabs value={String(topModelsLimit)} onValueChange={(value: string) => setTopModelsLimit(Number(value))}>
          <TabsList
            aria-label={t("usagePage.topModelView.numberOfModelsAria", { defaultValue: "Number of models to show" })}
          >
        ...
        <Tabs value={modelViewMode} onValueChange={(value: string) => setModelViewMode(value as "chart" | "table")}>
          <TabsList aria-label={t("usagePage.topModelView.viewModeAria", { defaultValue: "Top model view mode" })}>
            <TabsTrigger value="table" className="flex-none px-3">
              {t("usagePage.topModelView.tableView", { defaultValue: "Table View" })}
            </TabsTrigger>
            <TabsTrigger value="chart" className="flex-none px-3">
              {t("usagePage.topModelView.chartView", { defaultValue: "Chart View" })}
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
