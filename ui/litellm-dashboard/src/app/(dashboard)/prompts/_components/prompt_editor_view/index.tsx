...
const PromptEditorView: React.FC<PromptEditorViewProps> = ({ onClose, onSuccess, accessToken, initialPromptData }) => {
  ...
  const getInitialPrompt = (): PromptType => {
    if (initialPromptData) {
      try {
        ...
        toast.fromError(
          t("promptsPage.promptEditorView.failedToParsePrompt", { defaultValue: "Failed to parse prompt data" }),
        );
      }
    }
    return {
  ...
  const addTool = (json: string) => {
    try {
      ...
      toast.fromError(t("promptsPage.promptEditorView.invalidJsonFormat", { defaultValue: "Invalid JSON format" }));
    }
  };
  ...
  const handleLoadVersion = (versionData: any) => {
    try {
      ...
      toast.fromError(
        t("promptsPage.promptEditorView.failedToLoadVersion", { defaultValue: "Failed to load prompt version" }),
      );
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (!prompt.name || prompt.name.trim() === "") {
      toast.fromError(
        t("promptsPage.promptEditorView.invalidPromptName", { defaultValue: "Please enter a valid prompt name" }),
      );
      return;
    }
    ...
    try {
      ...
      if (editMode && initialPromptData?.prompt_spec?.prompt_id) {
        ...
        toast.success(
          t("promptsPage.promptEditorView.updateSuccess", { defaultValue: "Prompt updated successfully!" }),
        );
      } else {
      ...
      toast.fromError(
        editMode
          ? t("promptsPage.promptEditorView.updateFailed", { defaultValue: "Failed to update prompt" })
          : t("promptsPage.promptEditorView.saveFailed", { defaultValue: "Failed to save prompt" }),
      );
    } finally {
      setIsSaving(false);
          ...
          <div className="w-1/2 overflow-y-auto bg-card border-r border-border shrink-0">
            <div className="border-b border-border bg-card px-6 py-4 flex items-center gap-3">
              ...
              <div className="ml-auto inline-flex items-center bg-border rounded-full p-0.5">
                ...
                >
                  {t("promptsPage.promptEditorView.viewModePretty", { defaultValue: "PRETTY" })}
                </button>
                <button
                  className={`px-3 py-1 text-xs font-medium rounded-full transition-colors ${
                ...
                >
                  {t("promptsPage.promptEditorView.viewModeDotprompt", { defaultValue: "DOTPROMPT" })}
                </button>
              </div>
            </div>
