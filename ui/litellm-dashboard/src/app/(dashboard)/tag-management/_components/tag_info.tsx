...
const tagEditShape = (t: TFunction) => ({
  name: z.string().min(1, t("tagManagement.tagInfo.tagNameRequired", { defaultValue: "Please input a tag name" })),
...
const tagEditSchema = (t: TFunction) => z.object(tagEditShape(t));
...
const TagEditForm: React.FC<TagEditFormProps> = ({ tag, seedBudgetFields, userModels, onCancel, onSave }) => {
  ...
  const schema = React.useMemo(() => tagEditSchema(t), [t]);
    ...
    <form onSubmit={form.handleSubmit(submitVisibleValues)} noValidate>
      <FieldGroup>
        ...
        <FormField
          control={form.control}
          name="models"
          label={t("tagManagement.tagInfo.allowedModelsLabel", { defaultValue: "Allowed Models" })}
          description={t("tagManagement.tagInfo.allowedModelsTooltip", {
            defaultValue: "Select which models are allowed to process this type of data",
          })}
        >
          {({ value, onChange }) => (
            <MultiSelect
              ...
              placeholder={t("tagManagement.tagInfo.selectModelsPlaceholder", { defaultValue: "Select Models" })}
            />
          )}
        </FormField>
      </FieldGroup>
      ...
      >
        <CollapsibleTrigger className="group flex w-full items-center justify-between px-4 py-3 text-base font-medium text-foreground">
          {t("tagManagement.tagInfo.budgetRateLimits", { defaultValue: "Budget & Rate Limits" })}
        ...
        <CollapsibleContent className="px-4 pb-4">
          <FieldGroup className="mt-4">
            <FormField
              control={form.control}
              name="max_budget"
              label={t("tagManagement.tagInfo.maxBudgetLabel", { defaultValue: "Max Budget (USD)" })}
              description={t("tagManagement.tagInfo.maxBudgetTooltip", {
                defaultValue: "Maximum amount in USD this tag can spend",
              })}
            >
            ...
            <FormField
              ...
              label={t("tagManagement.tagInfo.resetBudgetLabel", { defaultValue: "Reset Budget" })}
              description={t("tagManagement.tagInfo.resetBudgetTooltip", {
                defaultValue: "How often the budget should reset",
              })}
            >
              {({ id, value, onChange }) => (
      ...
      <div className="flex justify-end space-x-2">
        <Button type="button" variant="outline" onClick={onCancel}>
          {t("tagManagement.tagInfo.cancelButton", { defaultValue: "Cancel" })}
        </Button>
        <Button type="submit">{t("tagManagement.tagInfo.saveChanges", { defaultValue: "Save Changes" })}</Button>
      </div>
    </form>
  );
};
...
const TagInfoView: React.FC<TagInfoViewProps> = ({ tagId, onClose, accessToken, is_admin, editTag }) => {
  ...
  const fetchTagDetails = async () => {
    ...
    try {
      ...
      toast.fromError(
        t("tagManagement.tagInfo.errorFetchingTagDetails", {
          error,
          defaultValue: "Error fetching tag details: {{error}}",
        }),
      );
    }
  };
  ...
  const handleSave = async (values: TagEditFormValues) => {
    ...
    try {
      ...
      toast.success(t("tagManagement.tagInfo.tagUpdatedSuccess", { defaultValue: "Tag updated successfully" }));
      setIsEditing(false);
      fetchTagDetails();
    } catch (error) {
      console.error("Error updating tag:", error);
      toast.fromError(
        t("tagManagement.tagInfo.errorUpdatingTag", { error, defaultValue: "Error updating tag: {{error}}" }),
      );
    }
  };

  if (!tagDetails) {
    return <div>{t("tagManagement.tagInfo.loading", { defaultValue: "Loading..." })}</div>;
  }
    ...
    <div className="p-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          <Button onClick={onClose} className="mb-4">
            {t("tagManagement.tagInfo.backToTags", { defaultValue: "← Back to Tags" })}
          </Button>
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">
              {t("tagManagement.tagInfo.tagNameLabel", { defaultValue: "Tag Name:" })}
            </span>
          ...
          <p className="text-sm text-muted-foreground">
            {tagDetails.description || t("tagManagement.tagInfo.noDescription", { defaultValue: "No description" })}
          </p>
        </div>
        {is_admin && !isEditing && (
          <Button onClick={() => setIsEditing(true)}>
            {t("tagManagement.tagInfo.editTagButton", { defaultValue: "Edit Tag" })}
          </Button>
        )}
      </div>
      ...
      {isEditing ? (
        ...
        <div className="space-y-6">
          <Card>
            <CardContent>
              <CardTitle>{t("tagManagement.tagInfo.tagDetails", { defaultValue: "Tag Details" })}</CardTitle>
              <div className="space-y-4 mt-4">
                <div>
                  <p className="font-medium">{t("guardrails.guardrailTable.colName", { defaultValue: "Name" })}</p>
                  <p>{tagDetails.name}</p>
                </div>
                <div>
                  <p className="font-medium">
                    {t("guardrails.keywordTable.colDescription", { defaultValue: "Description" })}
                  </p>
                  <p>{tagDetails.description || "-"}</p>
                </div>
                <div>
                  <p className="font-medium">
                    {t("tagManagement.tagInfo.allowedModelsLabel", { defaultValue: "Allowed Models" })}
                  </p>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {!tagDetails.models || tagDetails.models.length === 0 ? (
                      <Badge variant="secondary">
                        {t("tagManagement.tagInfo.allModels", { defaultValue: "All Models" })}
                      </Badge>
                    ) : (
                ...
                <div>
                  <p className="font-medium">{t("tagManagement.tagInfo.created", { defaultValue: "Created" })}</p>
                  <p>{tagDetails.created_at ? new Date(tagDetails.created_at).toLocaleString() : "-"}</p>
                </div>
                <div>
                  <p className="font-medium">
                    {t("tagManagement.tagInfo.lastUpdated", { defaultValue: "Last Updated" })}
                  </p>
                ...
                <div className="space-y-4 mt-4">
                  {tagDetails.litellm_budget_table.max_budget !== undefined &&
                    tagDetails.litellm_budget_table.max_budget !== null && (
                      <div>
                        <p className="font-medium">
                          {t("tagManagement.tagInfo.maxBudget", { defaultValue: "Max Budget" })}
                        </p>
                  ...
                  {tagDetails.litellm_budget_table.budget_duration && (
                    <div>
                      <p className="font-medium">
                        {t("tagManagement.tagInfo.budgetDuration", { defaultValue: "Budget Duration" })}
                      </p>
                  ...
                  {tagDetails.litellm_budget_table.tpm_limit !== undefined &&
                    tagDetails.litellm_budget_table.tpm_limit !== null && (
                      <div>
                        <p className="font-medium">
                          {t("tagManagement.tagInfo.tpmLimit", { defaultValue: "TPM Limit" })}
                        </p>
                  ...
                  {tagDetails.litellm_budget_table.rpm_limit !== undefined &&
                    tagDetails.litellm_budget_table.rpm_limit !== null && (
                      <div>
                        <p className="font-medium">
                          {t("tagManagement.tagInfo.rpmLimit", { defaultValue: "RPM Limit" })}
                        </p>
