...
const CodeInterpreterTool: React.FC<CodeInterpreterToolProps> = ({
  ...
  const handleToggle = (checked: boolean) => {
    if (checked && !isOpenAI) {
      toast.warning(
        t("playground.codeInterpreterTool.onlyOpenAIWarning", {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-foreground">
            {t("playground.codeInterpreterTool.title", { defaultValue: "Code Interpreter" })}
          </span>
          <Tooltip>
            <TooltipTrigger
              aria-label={t("playground.codeInterpreterTool.aboutTitle", { defaultValue: "About Code Interpreter" })}
            >
              <Info className="size-3 text-muted-foreground" />
            </TooltipTrigger>
            <TooltipContent>
              {t("playground.codeInterpreterTool.tooltipDesc", {
                defaultValue:
        ...
        <Switch
          ...
          disabled={isDisabled}
          size="sm"
          aria-label={t("playground.chatUi.enableCodeInterpreterTooltip", { defaultValue: "Enable Code Interpreter" })}
        />
      </div>
        ...
        <div className="mt-2 pt-2 border-t border-border">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-muted-foreground">
              <span>
                {t("playground.codeInterpreterTool.onlyOpenAINotice", {
              ...
              >
                {t("playground.codeInterpreterTool.requestSupportLink", {
                  defaultValue: "Request support for other providers",
                })}
              </a>
            </div>
          </div>
