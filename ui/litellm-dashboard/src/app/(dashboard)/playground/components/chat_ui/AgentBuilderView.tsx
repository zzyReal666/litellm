    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-1">
          {t("playground.agentBuilderView.proxyBaseUrl", { defaultValue: "Proxy base URL" })}
        </h3>
      ...
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-2">
          {t("playground.agentBuilderView.callYourAgent", { defaultValue: "Call your agent (cURL)" })}
        </h3>
        <CodeBlock code={curlExample} language="bash" />
      </div>
      <div className="rounded-lg border border-border bg-muted p-4">
        <h3 className="text-sm font-semibold text-foreground mb-2">
          {t("playground.agentBuilderView.createKeyTitle", { defaultValue: "Create a key for this agent" })}
        </h3>
        ...
        <Button onClick={onCreateKey} disabled={creatingKey || disabledPersonalKeyCreation}>
          {t("playground.agentBuilderView.createKeyButton", { defaultValue: "Create key for this agent" })}
        </Button>
        {disabledPersonalKeyCreation && (
          <p className="text-xs text-warning mt-2">
            {t("playground.agentBuilderView.keyCreationDisabled", {
              defaultValue: "Key creation is disabled for your account.",
            })}
          </p>
        )}
        {createdKeyValue && (
...
export default function AgentBuilderView({
  ...
  const loadAgents = useCallback(async (): Promise<AgentModel[]> => {
    ...
    try {
      ...
      console.error(e);
      toast.fromError(t("playground.agentBuilderView.failedToLoadAgents", { defaultValue: "Failed to load agents" }));
      return [];
    } finally {
  ...
  const handleSaveAgent = async () => {
    if (!accessToken || !draftName?.trim() || !draftUnderlyingModel) {
      toast.fromError(
        t("playground.agentBuilderView.nameAndModelRequired", {
          defaultValue: "Name and underlying model are required",
        }),
      );
      return;
    }
    ...
    try {
      ...
      toast.fromError(t("playground.agentBuilderView.failedToSaveAgent", { defaultValue: "Failed to save agent" }));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateAgent = async () => {
    ...
    try {
      ...
      toast.success(t("playground.agentBuilderView.agentUpdated", { defaultValue: "Agent updated successfully" }));
      const list = await loadAgents();
      ...
      toast.fromError(t("playground.agentBuilderView.failedToUpdateAgent", { defaultValue: "Failed to update agent" }));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCreateKeyForAgent = async () => {
    ...
    try {
      ...
      if (keyValue) {
        ...
        toast.success(
          t("playground.agentBuilderView.virtualKeyCreated", {
            defaultValue: "Virtual key created. Use it in the curl example below.",
          }),
        );
      } else {
        toast.fromError(
          t("playground.agentBuilderView.keyCreatedNoValue", { defaultValue: "Key created but value not returned" }),
        );
      }
    } catch (e) {
      toast.fromError(
        t("playground.agentBuilderView.failedToCreateKey", { defaultValue: "Failed to create key for agent" }),
      );
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("playground.agentBuilderView.agentDeleted", { defaultValue: "Agent deleted" }));
      const list = await loadAgents();
      ...
      toast.fromError(t("playground.agentBuilderView.failedToDeleteAgent", { defaultValue: "Failed to delete agent" }));
    } finally {
      setDeleting(false);
  ...
  if (!accessToken || !userID || !userRole) {
    return (
      <div className="flex h-full items-center justify-center p-8 text-muted-foreground">
        {t("playground.agentBuilderView.signInRequired", { defaultValue: "Sign in to use Agent Builder." })}
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col bg-card text-foreground">
      <div className="flex shrink-0 flex-col border-b border-border">
        <div className="flex h-12 items-center justify-between px-4">
          <span className="text-sm font-medium text-foreground">
            {t("playground.agentBuilderView.title", { defaultValue: "Agent Builder" })}
          </span>
          {isNewAgent ? (
            <Button onClick={handleSaveAgent} disabled={saving || !draftName?.trim() || !draftUnderlyingModel}>
              <Save />
              {t("playground.agentBuilderView.saveAgent", { defaultValue: "Save Agent" })}
            </Button>
          ) : (
            <span className="text-xs text-muted-foreground">
              {t("playground.agentBuilderView.buildAgentsSubtitle", {
                defaultValue: "Build Agents that pass your compliance requirements.",
              })}
            </span>
          )}
        </div>
        ...
        <div className="w-60 shrink-0 border-r border-border bg-card flex flex-col">
          <div className="flex items-center justify-between border-b border-border p-3">
            <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              {t("playground.agentBuilderView.agentsLabel", { defaultValue: "Agents" })}
            </span>
            <Button
              variant="ghost"
              size="icon-sm"
              onClick={handleAddAgent}
              aria-label={t("playground.agentBuilderView.addAgentAriaLabel", { defaultValue: "Add agent" })}
            >
              <Plus />
            </Button>
          </div>
            ...
            {loadingAgents ? (
              ...
              <>
                ...
                >
                  ...
                  {t("playground.agentBuilderView.newAgent", { defaultValue: "New agent" })}
                </button>
              </>
            )}
          </div>
          ...
          {selectedId === null && !isNewAgent && agentModels.length === 0 && !loadingAgents && (
            <div className="flex flex-1 items-center justify-center p-8 text-muted-foreground">
              {t("playground.agentBuilderView.noAgentsYet", {
                defaultValue: "No agents yet. Add an agent to get started.",
              })}
            </div>
          )}
                      ...
                      <div className="mx-auto max-w-xl space-y-4">
                        ...
                        <div>
                          <label className="mb-1 block text-sm font-medium text-foreground">
                            {t("playground.agentBuilderView.agentNameLabel", { defaultValue: "Agent name" })}
                          </label>
                          <Input
                            value={draftName}
                        ...
                        <div>
                          <label className="mb-1 block text-sm font-medium text-foreground">
                            {t("playground.agentBuilderView.systemPromptLabel", { defaultValue: "System prompt" })}
                          </label>
                          <Textarea
                        ...
                        <div>
                          <label className="mb-1 block text-sm font-medium text-foreground">
                            {t("playground.agentBuilderView.underlyingLlmLabel", { defaultValue: "Underlying LLM" })}
                          </label>
                          <Select
                          ...
                          >
                            <SelectTrigger
                              className="w-full"
                              aria-label={t("playground.agentBuilderView.underlyingLlmLabel", {
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="mb-1 block text-sm font-medium text-foreground">
                              {t("playground.agentBuilderView.temperatureLabel", { defaultValue: "Temperature" })}
                            </label>
                            <Input
                              type="number"
                              min={0}
                          ...
                          <div>
                            <label className="mb-1 block text-sm font-medium text-foreground">
                              {t("playground.agentBuilderView.maxTokensLabel", { defaultValue: "Max tokens" })}
                            </label>
                            <Input
                              type="number"
                              min={1}
                        ...
                        <div>
                          <label className="mb-1 block text-sm font-medium text-foreground">
                            {t("playground.agentBuilderView.mcpServersLabel", { defaultValue: "MCP servers" })}
                          </label>
                          <MultiSelect
                            placeholder={t("playground.agentBuilderView.mcpServersPlaceholder", {
                              defaultValue: "Select MCP servers to attach (same format as chat completions API)",
                            })}
                          ...
                          <div className="flex flex-wrap items-center gap-2 pt-2">
                            {selectedAgentModelId && (
                              <>
                                ...
                                >
                                  <Save />
                                  {t("playground.agentBuilderView.updateAgent", { defaultValue: "Update Agent" })}
                                </Button>
                            ...
                            <Button onClick={() => goToTab("chat")}>
                              <MessageSquare />
                              {t("playground.agentBuilderView.testInChat", { defaultValue: "Test in Chat" })}
                            </Button>
                          </div>
                        )}
                      </div>
                    ...
                    {selectedAgent ? (
                      ...
                      <div className="flex flex-1 items-center justify-center text-muted-foreground">
                        {t("playground.agentBuilderView.saveAgentFirst", {
                          defaultValue: "Save an agent first to test in Chat.",
                        })}
                      </div>
                    )}
                  </div>
                    ...
                    {selectedAgent ? (
                      ...
                      <div className="flex flex-1 items-center justify-center text-muted-foreground">
                        {t("playground.agentBuilderView.selectAgentForBatchTest", {
                          defaultValue: "Select an agent to run batch tests.",
                        })}
                      </div>
                    )}
                  </div>
                    ...
                    {selectedAgent ? (
                      ...
                      <div className="flex flex-1 items-center justify-center text-muted-foreground">
                        {t("playground.agentBuilderView.selectAgentToConnect", {
                          defaultValue: "Select an agent to see how to connect.",
                        })}
                      </div>
                    )}
                  </div>
      ...
      <AlertDialog open={confirmingDelete} onOpenChange={setConfirmingDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("playground.agentBuilderView.deleteAgentTitle", { defaultValue: "Delete agent" })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              <Trans
                i18nKey="playground.agentBuilderView.deleteAgentConfirm"
                values={{ name: selectedAgent?.model_name }}
                defaults={'Are you sure you want to delete "{{name}}"? This cannot be undone.'}
              />
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
