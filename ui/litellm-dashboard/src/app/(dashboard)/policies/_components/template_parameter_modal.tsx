    ...
    <Dialog open={visible} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="sm:max-w-175">
        <DialogHeader>
          ...
          <DialogDescription>
            {t("policies.templateParameterModal.subtitle", {
              defaultValue: "Configure competitor blocking for your brand",
            })}
          </DialogDescription>
        </DialogHeader>
            ...
            <>
              <div>
                <label className="mb-2 block text-sm font-medium">
                  {t("policies.templateParameterModal.competitorDiscovery", { defaultValue: "Competitor Discovery" })}
                </label>
                <RadioGroup
                ...
                >
                  <label className="flex cursor-pointer items-center justify-center gap-2 rounded-md border border-input px-3 py-2 text-sm">
                    ...
                    {`✨ ${t("policies.templateParameterModal.useAi", { defaultValue: "Use AI" })}`}
                  </label>
                  <label className="flex cursor-pointer items-center justify-center gap-2 rounded-md border border-input px-3 py-2 text-sm">
                    <RadioGroupItem value="manual" />
                    {t("policies.templateParameterModal.enterManually", { defaultValue: "Enter Manually" })}
                  </label>
                </RadioGroup>
              </div>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">
                  {t("policies.templateParameterModal.brandName", { defaultValue: "Your Brand Name" })}
                  <span className="ml-1 text-destructive">*</span>
                </label>
                <Input
                  placeholder={t("policies.templateParameterModal.brandNamePlaceholder", {
                    defaultValue: "e.g. Acme Airlines",
                  })}
              ...
              {competitorMode === "ai" && (
                <>
                  <div>
                    <label className="mb-1 block text-sm font-medium">
                      {t("policies.templateParameterModal.selectModel", { defaultValue: "Select Model" })}
                      <span className="ml-1 text-destructive">*</span>
                    </label>
                    <SearchSelect
                      ...
                      placeholder={
                        isLoadingModels
                          ...
                          : t("policies.templateParameterModal.selectModelPlaceholder", {
                              defaultValue: "Select a model to generate names",
                            })
                      }
                      emptyText={t("modelSelect.paginatedModelSelect.noModelsFound", {
                  ...
                  >
                    {isGenerating
                      ? `✨ ${t("policies.templateParameterModal.generatingNames", { defaultValue: "Generating names..." })}`
                      : `✨ ${t("policies.templateParameterModal.generateCompetitorNames", {
                          defaultValue: "Generate Competitor Names",
                        })}`}
                  </Button>
                </>
              )}
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">
                  {t("policies.templateParameterModal.competitorNames", { defaultValue: "Competitor Names" })}
                ...
                <div className="flex flex-wrap items-center gap-1.5 rounded-md border border-input p-2">
                  {competitorTags.map((tag) => (
                    <Badge key={tag} variant="secondary" className="gap-1">
                      ...
                      <button
                        type="button"
                        aria-label={t("policies.templateParameterModal.removeCompetitor", {
                          name: tag,
                  ...
                  <input
                    ...
                    placeholder={t("policies.templateParameterModal.competitorNamesPlaceholder", {
                      defaultValue: "Type a name and press Enter to add",
                    })}
                    value={tagDraft}
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("policies.templateParameterModal.competitorNamesHint", {
                    defaultValue: "Type a name and press Enter to add. Click ✕ to remove.",
                  })}
                </p>
                {statusMessage && (
                ...
                {Object.keys(variationsMap).length > 0 && !statusMessage && (
                  <p className="mt-1 text-xs text-success">
                    {t("policies.templateParameterModal.variationsGenerated", {
                      ...
                      defaultValue:
                        "✓ {{count}} alternate spellings & variations auto-generated for guardrail matching",
                    })}
                  </p>
                )}
              </div>
              ...
              {competitorMode === "ai" && hasGenerated && competitorTags.length > 0 && (
                <div>
                  <label className="mb-1 block text-sm font-medium">
                    {t("policies.templateParameterModal.refineList", { defaultValue: "Refine List" })}
                  </label>
                  <div className="flex gap-2">
                    <Input
                      placeholder={t("policies.templateParameterModal.refinePlaceholder", {
                        defaultValue: "e.g. add 10 more from Asia, increase to 50 total...",
                      })}
                    ...
                    <Button onClick={handleRefine} disabled={!refinementInput.trim() || isRefining} size="sm">
                      {isRefining ? "..." : t("policies.templateParameterModal.send", { defaultValue: "Send" })}
                    </Button>
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t("policies.templateParameterModal.refineHint", {
                      defaultValue: "Give instructions to add, remove, or change competitors. Press Enter to send.",
                    })}
                  </p>
                </div>
              )}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={!canContinue || isLoading}>
            {isLoading
              ? t("policies.templateParameterModal.creatingGuardrails", { defaultValue: "Creating guardrails..." })
              : t("policies.templateParameterModal.continue", { defaultValue: "Continue" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
