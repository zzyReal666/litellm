    ...
    >
      <TooltipProvider>
        <Tooltip>
              ...
              <PopoverTrigger
                render={
                  <Button
                    variant="ghost"
                    size="icon-xs"
                    aria-label={t("policies.impactPopover.viewBlastRadius", { defaultValue: "View blast radius" })}
                  >
                    <Eye />
                  </Button>
                }
          ...
          />
          <TooltipContent>
            {t("policies.impactPopover.viewBlastRadius", { defaultValue: "View blast radius" })}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>

      <PopoverContent className="w-72 gap-2">
        <PopoverTitle>{t("policies.impactPopover.blastRadius", { defaultValue: "Blast Radius" })}</PopoverTitle>
        {loading ? (
            ...
            {impact.affected_keys_count === -1 ? (
              <p className="font-medium text-foreground">
                {t("policies.impactPopover.globalScope", {
                  defaultValue: "Global scope — affects all keys and teams",
                })}
              </p>
            ) : (
              <>
                <p className="mb-1">
                  <Trans
                    ...
                    defaults="<0>{{keysCount}}</0> {{keysWord}}, <1>{{teamsCount}}</1> {{teamsWord}} affected"
                    values={{
                      keysCount: impact.affected_keys_count,
                      keysWord: t("policies.impactPopover.keyWord", {
                        count: impact.affected_keys_count,
                        defaultValue: "keys",
                      }),
                      teamsCount: impact.affected_teams_count,
                      teamsWord: t("policies.impactPopover.teamWord", {
                        count: impact.affected_teams_count,
                        defaultValue: "teams",
                      }),
                    }}
                ...
                {impact.sample_keys.length > 0 && (
                  <div className="mb-1 flex flex-wrap items-center gap-1">
                    <span className="text-muted-foreground">
                      {t("policies.impactPopover.keysLabel", { defaultValue: "Keys:" })}
                    </span>
                ...
                {impact.sample_teams.length > 0 && (
                  <div className="flex flex-wrap items-center gap-1">
                    <span className="text-muted-foreground">
                      {t("policies.impactPopover.teamsLabel", { defaultValue: "Teams:" })}
                    </span>
                ...
                {impact.affected_keys_count === 0 && impact.affected_teams_count === 0 && (
                  <p className="text-muted-foreground">
                    {t("policies.impactPopover.noKeysOrTeams", { defaultValue: "No keys or teams currently affected" })}
                  </p>
                )}
              </>
            )}
          ...
          <p className="text-xs text-muted-foreground">
            {t("policies.impactPopover.clickToLoad", { defaultValue: "Click to load" })}
          </p>
        )}
      </PopoverContent>
    </Popover>
