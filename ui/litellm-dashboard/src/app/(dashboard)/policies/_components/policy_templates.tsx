      ...
      <CardContent className="flex h-full flex-col">
        <div className="mb-4 flex items-start justify-between">
          ...
          <Badge variant="outline">
            {t("policies.policyTemplates.complexityLabel", {
              complexity,
        ...
        {inherits && (
          <div className="mb-4 text-xs">
            <span className="text-muted-foreground">
              {t("policies.policyTemplates.inheritsFrom", { defaultValue: "Inherits from:" })}{" "}
            </span>
        ...
        <div className="mb-6">
          <span className="mb-2 block text-xs font-medium tracking-wider text-muted-foreground uppercase">
            {t("policies.policyTemplates.includedGuardrails", { defaultValue: "Included Guardrails" })}
          </span>
        ...
        <Button className="mt-auto w-full" onClick={onUseTemplate}>
          {t("policies.policyTemplates.useTemplate", { defaultValue: "Use Template" })}
        </Button>
      </CardContent>
    </Card>
  );
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        ...
        toast.error(t("policies.policyTemplates.fetchFailed", { defaultValue: "Failed to fetch policy templates" }));
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-lg font-medium">
            {t("policies.policyTemplates.title", { defaultValue: "Policy Templates" })}
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("policies.policyTemplates.subtitle", {
              defaultValue:
        ...
        <Button variant="outline" onClick={onOpenAiSuggestion}>
          ...
          {t("policies.policyTemplates.useAiToFind", { defaultValue: "Use AI to find templates" })}
        </Button>
      </div>
            ...
            <div className="sticky top-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold">
                  {t("policies.policyTemplates.categories", { defaultValue: "Categories" })}
                </span>
                {selectedTags.size > 0 && (
                  <button onClick={handleClearAll} className="text-xs text-primary hover:underline">
                    {t("policies.policyTemplates.clearAll", { defaultValue: "Clear all" })}
                  </button>
                )}
              </div>
        ...
        <div className="flex-1">
          {selectedTags.size > 0 && (
            <div className="mb-4 text-sm text-muted-foreground">
              {t("policies.policyTemplates.showingCount", {
                ...
                defaultValue: "Showing {{filtered}} of {{total}} templates",
              })}
            </div>
          )}
          ...
          {filteredTemplates.length === 0 && (
            <div className="py-12 text-center text-muted-foreground">
              <p>
                {t("policies.policyTemplates.noTemplatesMatch", {
                  defaultValue: "No templates match the selected filters.",
                })}
              </p>
              <button onClick={handleClearAll} className="mt-2 text-sm text-primary hover:underline">
                {t("policies.policyTemplates.clearAllFilters", { defaultValue: "Clear all filters" })}
              </button>
            </div>
          )}
        </div>
