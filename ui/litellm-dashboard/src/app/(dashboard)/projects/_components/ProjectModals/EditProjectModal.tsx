...
function EditProjectForm({ project, onClose, onSuccess }: Omit<EditProjectModalProps, "isOpen">) {
  ...
  const handleSubmit = form.handleSubmit((values) => {
    ...
    updateMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(t("projects.editProjectModal.updateSuccess", { defaultValue: "Project updated successfully" }));
          onSuccess?.();
          onClose();
        },
        onError: (error) => {
          toast.error(
            error.message || t("projects.editProjectModal.updateFailed", { defaultValue: "Failed to update project" }),
          );
        },
      },
    );
    ...
    <form onSubmit={(event) => event.preventDefault()}>
      ...
      <div className="mt-6 flex justify-end gap-2 border-t border-border pt-4">
        ...
        <Button type="button" onClick={() => void handleSubmit()} disabled={updateMutation.isPending}>
          ...
          {t("projects.editProjectModal.saveBtn", { defaultValue: "Save Changes" })}
        </Button>
      </div>
    </form>
  );
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[720px]">
        <DialogHeader>
          <DialogTitle className="text-lg">
            {t("projects.editProjectModal.title", { defaultValue: "Edit Project" })}
          </DialogTitle>
        </DialogHeader>
