...
| 版本 | 日期 | 变更摘要 | 修改人 |
|---|---|---|---|
| v1.8 | 2026-09-15 | 按网关 v1.100.0 重做截图；`deepseek-chat` 已停止服务，模型表改为 `deepseek-v4-flash` / `deepseek-v4-pro`；`reasoning_content` 工具调用兼容问题已解决，5.4 相应条目作废；邀请链接地址更新为 `/ui/onboarding` | 张忠源 |
...
| 项目 | v1.7 手册 | v1.8 实际状态 |
|---|---|---|
| `deepseek-chat` | 可用，计划 2026/07/24 弃用 | 该模型名已停止服务，调用返回 400。需要这个模型时改用 `deepseek/deepseek-chat` |
| `reasoning_content` 工具调用报错 | Cursor 等工具多轮 Agent 对话会报 400 | 网关上已验证不再强制回传，带与不带 `reasoning_content` 都能正常完成多轮工具调用 |
| 邀请链接地址 | `http://192.168.7.99:4000/ui?invitation_id=...` | `http://192.168.7.99:4000/ui/onboarding?invitation_id=...` |
| 中文管理后台（端口 80） | 可用 | 已于 2026-09-10 下线，统一使用内置后台 http://192.168.7.99:4000/ui/ |
| 查看可用模型 | 在 Models + Endpoints 页面核对 | 该页面是模型管理页，普通用户看到的不是模型清单。改用 `GET /v1/models` 或按 5.1 的固定模型名 |
...
| 模型名 | 说明 |
|---|---|
| `deepseek-v4-flash` | 快速版，适合日常对话、代码补全等对响应速度要求较高的场景 |
...
在管理后台为自己创建 Virtual Key（虚拟密钥），在代码、Cursor、Dify 等工具里调用
...
页面标题为 Sign Up（注册），说明为 Claim your user account to login to Admin UI

![](user-manual-v1.8/03-1-signup.png)

页面上的 SSO 入口属于企业版功能，当前未启用，忽略即可

### 3.2 设置登录密码

![](user-manual-v1.8/03-2-signup-filled.png)
...
![](user-manual-v1.8/03-3-login.png)

使用邀请中的邮箱 + 刚设置的密码登录

登录后默认进入 Virtual Keys 页面，右上角显示你的邮箱，左侧菜单只包含你权限范围内的功能

![](user-manual-v1.8/03-4-home.png)

> **Warning**
>
> 默认凭据提示里写的 `admin` / `MASTER_KEY` 是给管理员用的，普通用户不要尝试，直接用你自己的邮箱和密码登录
...
| 忘记密码 | 联系管理员发送重置密码链接 |
| 登录后看不到 Virtual Keys 菜单，或看不到 `+ Create New Key` 按钮 | 联系管理员确认账号角色。只有 `Internal User (Create/Delete/View)` 角色才能创建密钥，`Internal User (View Only)` 只能查看 |

---

## 四、创建 Virtual Key（API 密钥）

### 4.1 新建密钥

登录后，在左侧菜单进入 Virtual Keys（虚拟密钥）：

![](user-manual-v1.8/04-1-keys.png)

页面展示你名下的所有密钥，包含密钥别名、创建时间、最近使用时间、消费与额度、可用模型

1. 点击 `+ Create New Key` 按钮
2. 按表单填写

   ![](user-manual-v1.8/04-2-create-form.png)

   | 字段 | 填写说明 |
   |---|---|
   | Key Ownership - Owned By | 选择 `You`（你自己） |
   | Team | 不要选择 |
   | Key Name | 按命名规范填写：【姓名】的 key【编号】，例如「张三的 key1」「张三的 key2」 |
   | Models | 留空表示可访问管理员为你开通的全部模型 |
   | Key Type | 保持默认 `AI APIs` |
   | Optional Settings | 一般不用填，需要设预算或限流时再展开 |
...
![](user-manual-v1.8/04-3-key-saved.png)
...
也可以在 Virtual Keys 页面查看密钥行末尾的 Models 列，确认这把密钥被允许访问的模型范围

> **Warning**
>
> 不要用 `GET /v1/models` 作为可用模型的依据。该接口目前会返回 18 个名字，其中 `deepseek-chat`、`deepseek-reasoner`、`deepseek/deepseek-r1`、`deepseek/deepseek-v3.2` 等并没有实际可用的上游，调用一律返回 400。请以上表的四个名字为准

管理后台的 `Models + Endpoints` 页面是给管理员用的模型管理页，普通用户在那里看不到可用的模型清单，不要用它来核对模型名
  ...
  "choices": [
    {
      ...
      "message": {
        "role": "assistant",
        "content": "你好，我是 AI 助手，很高兴为你提供信息和帮助。"
      }
    }
  ],
  "usage": {
...
| 报错 403，提示 `User not allowed to access model` | 模型未对该用户开通 | 联系网关管理员开通对应模型 |
| 报错 401，提示 `Invalid proxy server token passed` | 密钥错误、已被删除或已过期 | 核对密钥是否完整复制；必要时在 Virtual Keys 页面重建 |
...
v1.7 手册里关于 `reasoning_content` 必须回传导致 400 的条目已在 v1.8 移除。2026-09-15 在网关上实测，`deepseek-v4-flash` 与 `deepseek-v4-pro` 的多轮工具调用，带与不带 `reasoning_content` 都能正常返回，不再需要为兼容该字段而回避 V4 系列
...
登录管理后台后，在左侧菜单进入 `Usage`，页面默认就是你的个人用量视图

![](user-manual-v1.8/06-1-usage.png)

Cost 标签页展示所选时间范围内的个人消费总额（Project Spend）、请求数、成功与失败请求数、平均单次成本、Token 总量，以及每日消费柱状图。页面继续往下是 Top Virtual Keys（名下各密钥的消费排名）、Top Public Model Names（各模型消费排名）与 Spend by Provider（按供应商汇总）

需要按调用类型细分时，切换到 `Key Activity`、`Endpoint Activity` 等标签页查看

在 `Virtual Keys` 页面也能直接看到每个密钥行的消费金额（Spend / Budget 列），以及最近一次使用时间
...
收到邀请链接
  ...
  -> 左侧菜单 Virtual Keys -> + Create New Key（Owned By 选 You，Key Name 按「姓名 + 的 key + 编号」）
  -> 弹出 Save your Key 窗口，立即 Copy Virtual Key 并妥善保存
